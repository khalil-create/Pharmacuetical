                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="form-group">
                            <form method="POST" action="/managerSales/UpdateMainArea/{{$area->id}}"  enctype="multipart/form-data">
                            {{ csrf_field() }}
                            {{method_field('PUT')}}
                            <div class="card-body">
                                <div class="form-group">
                                    ...
                                    <label for="supervisor_id" class="col-md-2 control-label">المشرف عليها</label>
                                        <select name="supervisor_id" id="supervisor_id" class="form-control custom-select rounded-0">
                                            ...
                                            @foreach ($supervisors as $sup)
                                                <option value="{{ $sup->supervisor->id }}" 
                                                        @if ($area->supervisor_id == $sup->supervisor->id)
                                                            {{ $selected }}
                                                        @endif    
                                                    >{{ $sup->user_name_third }} {{ $sup->user_surname }}
                                                </option>
                                            @endforeach
                                        </select>
