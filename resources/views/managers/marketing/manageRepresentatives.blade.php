    ...
    <div class="container-fluid">
      <div class="card card-default">
        <div class="card-header">
          <h3 class="card-title" style="float: right">قائمة المندوبين العلميين</h3>
              ...
              <table id="example1" class="table table-bordered table-striped dataTable dtr-inline" role="grid" aria-describedby="example1_info">
                ...
                @foreach ($rep as $row)
                  <tr class="odd">
                    ...
                    <td>
                      <a href="/managerMarketing/editRepresentative/{{$row->id}}"><i class="nav-icon fas fa-edit" title="تعديل"></i></a>
                      <a href="/managerMarketing/showSubareas/{{$row->id}}" title="مناطق هذا المندوب"><i class="fas fa-tasks"></i></a>
                      <a href="/managerMarketing/showRepDetails/{{$row->id}}" ><i class="fas fa-eye" title="تفاصيل اكثر"></i></a>
                      {{-- <form action="/managerMarketing/deleteRepresentative/{{$row->id}}" method="post" style="float: right;">
                          {{csrf_field()}}
                          {{method_field('DELETE')}}
                ...
                @endforeach
                <div>
                  <a href="{{url('/managerMarketing/addRepresentative')}}" class="btn btn-primary add"><i class="fas fa-plus"></i> اضافة مندوب</a>
                </div>
                </tbody>
                <tfoot>
                    <tr>
        ...
        $('.DeleteBtn').click(function(e){
            ...
            .then((willDelete) => {
                if (willDelete) {
                    ...
                    $.ajax({
                        type: "DELETE",
                        url: '/managerMarketing/deleteRepresentative/'+id,
                        data: data,
                        // dataType: "data"
