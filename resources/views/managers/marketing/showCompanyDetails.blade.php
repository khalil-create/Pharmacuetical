                            ...
                            <div class="info-box mb-3">
                                <div class="info-box-content">
                                    ...
                                    <span class="info-box-number">
                                        <a href="/managerMarketing/showSupervisorDetails/{{$company->supervisor->user_id}}" title="تفاصيل">
                    ...
                    <div class="row mt-1">
                        ...
                        <div class="tab-content p-3" id="nav-tabContent">
                            <div class="tab-pane fade show active" id="product-desc" role="tabpanel" aria-labelledby="product-desc-tab"><!-- categories -->
                                @if($company->categories->count() > 0)
                                    @foreach ($company->categories as $row)
                                        <a href="/managerMarketing/showCategoryDetails/{{$row->id}}" title="تفاصيل" class="btn btn-block btn-default btn-lg col-md-2"  style="height: 70px;margin:10px">
                            ...
                            <div class="tab-pane fade" id="product-comments" role="tabpanel" aria-labelledby="product-comments-tab"><!-- items -->
                                <div class="row">@php $count_items = 0; @endphp
                                    @if($company->have_category)
                                        @foreach ($company->categories as $cat)
                                            @if($cat->items->count() > 0)  
                                                @php $count_items+=$cat->items->count(); @endphp
                                            @endif
                                            @foreach ($cat->items as $item)
                                                <a href="/managerMarketing/showItemDetails/{{$row->id}}" title="تفاصيل" class="btn btn-block btn-default btn-lg col-md-2"  style="height: 70px;margin:10px">
                                                    <div class="col-md-12" style="margin:15px -8px 15px">
                                    ...
                                    @else
                                        @if($company->items->count() > 0)
                                            @foreach ($company->items as $item)
                                                <a href="/managerMarketing/showItemDetails/{{$item->id}}" title="تفاصيل" class="btn btn-block btn-default btn-lg col-md-3"  style="height: 70px;margin:10px">
                                                    <div class="col-md-12" style="margin:15px -8px 15px">
                                                        <h6>{{$item->commercial_name}}</h6>
                                                    </div>
                                                </a>
