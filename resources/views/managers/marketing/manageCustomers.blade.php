                ...
                <table id="example1" class="table table-bordered table-striped dataTable dtr-inline" role="grid" aria-describedby="example1_info">
                  ...
                  @foreach ($customers as $row)
                    <tr class="odd">
                      ...
                      <td class="text-right">
                        @foreach ($row->representative as $rep)
                            @if ($rep->user->user_type == 'مندوب علمي' || $rep->user->user_type == 'مدير فريق')
                                <p>
                                  <span class="text-success">م.علمي:-</span> {{$rep->user->user_name_third}} {{$rep->user->user_surname}}
                                </p>
                            @endif
                            @if ($rep->user->user_type == 'مندوب مبيعات')
                              <p>
                                <span class="text-success">م.مبيعات:-</span> {{$rep->user->user_name_third}} {{$rep->user->user_surname}}
                              </p>
                            @endif
                            {{-- @else
                                {{'----'}} --}}
                        @endforeach
                        {{-- {{$row->representative->first()->user->user_name_third}} {{$row->representative->first()->user->user_surname}} --}}
                      </td>
                      <td class="sorting_1">
                        @if ($row->type == 0)
                            {{'مستشفى'}}
                      ...
                      <td>
                        @if ($row->statues)
                          <a href="/managerMarketing/notActivateCustomer/{{$row->id}}" title="إلغاء التفعيل">
                            <b class="text-success">{{'مفعل'}}</b>
                          </a>
                        @else
                          <a href="/managerMarketing/activateCustomer/{{$row->id}}" title="تفعيل">
                            <b class="text-danger">{{'غير مفعل'}}</b>
                          </a>
                        @endif
                      </td>
                      <td>
                        <a href="/managerMarketing/editCustomer/{{$row->id}}"><i class="nav-icon fas fa-edit" title="تعديل"></i></a>
                        <a href="/managerMarketing/showCustomerDetails/{{$row->id}}"><i class="fas fa-eye" title="تفاصيل"></i></a>
                  ...
                  @endforeach
                  <div>
                    <a href="{{url('/managerMarketing/addCustomer')}}" class="btn btn-primary add"><i class="fas fa-plus"></i> اضافة عميل</a>
                  </div>
                  </tbody>
                  <tfoot>
                      <tr>
        ...
        $('.DeleteBtn').click(function(e){
            ...
            .then((willDelete) => {
                if (willDelete) {
                    ...
                    $.ajax({
                        type: "DELETE",
                        url: '/managerMarketing/deleteCustomer/'+id,
                        data: data,
                        // dataType: "data"
