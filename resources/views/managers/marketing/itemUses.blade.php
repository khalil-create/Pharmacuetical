                ...
                <table id="example1" class="table table-bordered table-striped dataTable dtr-inline" role="grid" aria-describedby="example1_info">
                  ...
                  @foreach ($item->uses as $row)               
                    <tr class="odd">
                      ...
                      <td>{{$row->use}}</td>
                      <td>
                        <a href="/managerMarketing/editUse/{{$row->id}}"><i class="nav-icon fas fa-edit" title="تعديل"></i></a>
                        {{-- <form action="/managerMarketing/deleteUse/{{$row->id}}" method="post" style="float: right;">
                            {{csrf_field()}}
                            {{method_field('DELETE')}}
                  ...
                  @endif
                  <div>
                    <a href="/managerMarketing/addUse/{{$item->id}}" class="btn btn-primary add"><i class="fas fa-plus"></i> اضافة استخدام</a>
                    {{-- @if($item->uses->count() > 0) --}}
                      <a href="/managerMarketing/addUseExist/{{$item->id}}" class="btn btn-primary add"><i class="fas fa-plus"></i> اضافة استخدامات موجودة</a>
                    {{-- @endif --}}
                  </div>
                  </tbody>
                  <tfoot>
        ...
        $('.DeleteBtn').click(function(e){
            ...
            .then((willDelete) => {
                if (willDelete) {
                    ...
                    $.ajax({
                        type: "DELETE",
                        url: '/managerMarketing/deleteUse/'+id,
                        data: data,
                        // dataType: "data"
