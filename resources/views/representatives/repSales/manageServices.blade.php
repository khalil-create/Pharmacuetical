                ...
                <table id="example1" class="table table-bordered table-striped dataTable dtr-inline" role="grid" aria-describedby="example1_info">
                  ...
                  @foreach ($services as $row)
                    <tr class="odd">
                      ...
                      <td>
                        <a href="/repSales/editService/{{$row->id}}"><i class="nav-icon fas fa-edit" title="تعديل"></i></a>
                        <i class="fas fa-eye"></i>
                        {{-- <form action="/repSales/deleteService/{{$row->id}}" method="post" style="float: right;">
                            {{csrf_field()}}
                            {{method_field('DELETE')}}
                  ...
                  @endforeach
                  <div>
                    <a href="{{url('/repSales/addService')}}" class="btn btn-primary add"><i class="fas fa-plus"></i> اضافة خدمة</a>
                  </div>
                  </tbody>
                  <tfoot>
                    @if($services->count() > 0)
