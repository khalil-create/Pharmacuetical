                ...
                <table id="example1" class="table table-bordered table-striped dataTable dtr-inline" role="grid" aria-describedby="example1_info">
                  ...
                  @foreach ($customers as $row)
                    <tr class="odd">
                      ...
                      <td>
                        @if ($row->statues)
                          <a href="/repSales/notActivateCustomer/{{$row->id}}" title="إلغاء التفعيل"><b style="color:#0bab30">{{'مفعل'}}</b></a>
                        @else
                          <a href="/repSales/activateCustomer/{{$row->id}}" title="تفعيل"><b style="color:hsl(0, 96%, 51%)">{{'غير مفعل'}}</b></a>
                        @endif
                      </td>
                      <td>
                        <a href="/repSales/editCustomer/{{$row->id}}"><i class="nav-icon fas fa-edit" title="تعديل"></i></a>
                  ...
                  @endforeach
                  <div>
                    <a href="{{url('/repSales/addCustomer')}}" class="btn btn-primary add"><i class="fas fa-plus"></i> اضافة عميل</a>
                  </div>
                  </tbody>
                  <tfoot>
                    @if($customers->count() > 0)
        ...
        $('.DeleteBtn').click(function(e){
            ...
            .then((willDelete) => {
                if (willDelete) {
                    ...
                    $.ajax({
                        type: "DELETE",
                        url: '/repSales/deleteCustomer/'+id,
                        data: data,
                        // dataType: "data"
