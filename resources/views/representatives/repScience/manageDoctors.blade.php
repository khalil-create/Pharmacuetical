      ...
      <div class="container-fluid">
        <div class="card card-default">
          <div class="card-header">
            <h3 class="card-title" style="float: right">قائمة الدكتور</h3>
                ...
                <table id="example1" class="table table-bordered table-striped dataTable dtr-inline" role="grid" aria-describedby="example1_info">
                  ...
                  @foreach ($doctors as $row)
                    <tr class="odd">
                      ...
                      <td>
                        <a href="/representative/editDoctor/{{$row->id}}"><i class="nav-icon fas fa-edit" title="تعديل"></i></a>
                        {{-- <a href="/representative/showMainareas/{{$row->id}}" ><i class="fas fa-tasks"></i></a> --}}
                        <i class="fas fa-eye"></i>
                        <form action="/representative/deleteDoctor/{{$row->id}}" method="post" style="float: right;">
                            {{csrf_field()}}
                            {{method_field('DELETE')}}
                  ...
                  @endforeach
                  <div>
                    <a href="{{url('/representative/addDoctor')}}" class="btn btn-primary add"><i class="fas fa-plus"></i> اضافة دكتور</a>
                    @if (session('status'))
                      <div class="alert alert-success notify-success">
                          {{ session('status') }}
                      </div>
