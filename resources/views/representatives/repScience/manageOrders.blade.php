                ...
                <table id="example1" class="table table-bordered table-striped dataTable dtr-inline" role="grid" aria-describedby="example1_info">
                  ...
                  @foreach ($orders as $row)
                    <tr class="odd">
                      ...
                      <td>{{$row->note}}</td>
                      <td>
                        <a href="/representative/editOrder/{{$row->id}}"><i class="nav-icon fas fa-edit" title="تعديل"></i></a>
                        <i class="fas fa-eye"></i>
                        <form action="/representative/deleteOrder/{{$row->id}}" method="post" style="float: right;">
                            {{csrf_field()}}
                            {{method_field('DELETE')}}
                  ...
                  @endforeach
                  <div>
                    <a href="{{url('/representative/addOrder')}}" class="btn btn-primary add"><i class="fas fa-plus"></i> اضافة طلبية</a>
                  </div>
                  </tbody>
                  <tfoot>
                    @if($orders->count() > 0)
