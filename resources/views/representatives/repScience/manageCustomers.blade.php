                ...
                <table id="example1" class="table table-bordered table-striped dataTable dtr-inline" role="grid" aria-describedby="example1_info">
                  ...
                  @foreach ($customers as $row)
                    <tr class="odd">
                      ...
                      <td>
                        <a href="/repScience/editCustomer/{{$row->id}}"><i class="nav-icon fas fa-edit" title="تعديل"></i></a>
                        <a href="/repScience/showCustomerDetails/{{$row->id}}"><i class="fas fa-eye"></i></a>
                        {{-- <form action="/repScience/deleteCustomer/{{$row->id}}" method="post" style="float: right;">
                            {{csrf_field()}}
                            {{method_field('DELETE')}}
                  ...
                  @endforeach
                  <div>
                    <a href="{{url('/repScience/addCustomer')}}" class="btn btn-primary add"><i class="fas fa-plus"></i> اضافة عميل</a>
                  </div>
                  </tbody>
                  <tfoot>
                    {{-- @if($customers->count() > 0) --}}
        ...
        $('.DeleteBtn').click(function(e){
            ...
            .then((willDelete) => {
                if (willDelete) {
                    ...
                    $.ajax({
                        type: "DELETE",
                        url: '/repScience/deleteCustomer/'+id,
                        data: data,
                        // dataType: "data"
