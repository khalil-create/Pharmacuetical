                ...
                <table id="example1" class="table table-bordered table-striped dataTable dtr-inline" role="grid" aria-describedby="example1_info">
                  ...
                  @foreach ($customers as $row)
                    <tr class="odd">
                      ...
                      <td>
                        @if ($row->statues)
                          <a href="/supervisor/notActivateCustomer/{{$row->id}}" title="إلغاء التفعيل"><b style="color:#0bab30">{{'مفعل'}}</b></a>
                        @else
                          <a href="/supervisor/activateCustomer/{{$row->id}}" title="تفعيل"><b style="color:hsl(0, 96%, 51%)">{{'غير مفعل'}}</b></a>
                        @endif
                      </td>
                      <td>
                        <a href="/supervisor/editCustomer/{{$row->id}}"><i class="nav-icon fas fa-edit" title="تعديل"></i></a>
                        <a href="/supervisor/showCustomerDetails/{{$row->id}}"><i class="fas fa-eye" title="تفاصيل"></i></a>
                  ...
                  @endforeach
                  <div>
                    <a href="{{url('/supervisor/addCustomer')}}" class="btn btn-primary add"><i class="fas fa-plus"></i> اضافة عميل</a>
                  </div>
                  </tbody>
                  <tfoot>
                    @if($customers->count() > 0)
        ...
        $('.DeleteBtn').click(function(e){
            ...
            .then((willDelete) => {
                if (willDelete) {
                    ...
                    $.ajax({
                        type: "DELETE",
                        url: '/supervisor/deleteCustomer/'+id,
                        data: data,
                        // dataType: "data"
