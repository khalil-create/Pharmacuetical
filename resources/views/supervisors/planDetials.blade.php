...
<div class="content-header content-wrapper">
  ...
  <div class="content">
    <div class="container-fluid">
      <div class="card card-default">
        <div class="card-header">
          <h3 class="card-title" style="float: right">اضافة بيانات الخطة لشهر ({{$plan->plan_month}})
            للمندوبـ/ـة:- {{$plan->represnetatives->user->user_name_third}} {{$plan->represnetatives->user->user_surname}}</h3>
        ...
        <div class="card-body">
          <div class="form-group">
            <form method="POST" action="/supervisor/storePlanCustomer/{{$plan->id}}" onsubmit="return checkDataPlan()"  enctype="multipart/form-data">
            {{ csrf_field() }}
            <div class="card-body">
              <div class="form-group">
                <div class="khalil">
                        ...
                        <div class="row">
                          <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="date" class="form-control" name="date" id="month_entered">
                                @if ($errors->has('date'))
                            ...
                            <input type="hidden" id="month_plan" value="{{$plan->plan_month}}">
                            <div class="form-group">
    ...
    <div class="card card-default">
      <div class="card-header">
        <h3 class="card-title" style="float: right">عرض بيانات الخطة لشهر ({{$plan->plan_month}})
          للمندوبـ/ـة:- {{$plan->represnetatives->user->user_name_third}} {{$plan->represnetatives->user->user_surname}}</h3>
        <div class="card-tools float-right">
          <button type="button" class="btn btn-tool" data-card-widget="collapse">
          <i class="fas fa-minus"></i>
          ...
          <table id="example1" class="table table-bordered table-striped dataTable dtr-inline" role="grid" aria-describedby="example1_info">
            ...
            @else
              <div class="alert alert-danger notify-error">
                {{ 'لم يتم اضافة اي خطة للمندوبـ/ـة: ' }}
                {{$plan->represnetatives->user->user_name_third}} {{$plan->represnetatives->user->user_surname}}
              </div>
            @endif
            </thead>
            <tbody>
            ...
            @foreach ($plan->customers_all as $row)
              <tr class="odd">
                ...
                <td>
                  {{-- <a href="/supervisor/editPlanCustomer/{{$row->id}}"><i class="nav-icon fas fa-edit" title="تعديل"></i></a> --}}
                  <a href="{{route('editPlanCustomer',['plan_id' => $row->id,'rep_id' => $plan->represnetatives->id])}}"><i class="nav-icon fas fa-edit" title="تعديل"></i></a>
        ...
        $('.DeleteBtn').click(function(e){
            ...
            .then((willDelete) => {
                if (willDelete) {
                    ...
                    $.ajax({
                        type: "DELETE",
                        url: '/supervisor/deletePlanCustomer/'+id,
                        data: data,
                        // dataType: "data"
