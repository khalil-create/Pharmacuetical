                ...
                <table id="example1" class="table table-bordered table-striped dataTable dtr-inline" role="grid" aria-describedby="example1_info">
                  ...
                  @foreach ($planTypes as $row)
                    <tr class="odd">
                      ...
                      <td>
                        <a href="/supervisor/editPlanType/{{$row->id}}"><i title="تعديل" class="nav-icon fas fa-edit"></i></a>
                        {{-- <form action="/supervisor/deletePlanType/{{$row->id}}" method="post" style="float: right;">
                          {{csrf_field()}}
                          {{method_field('DELETE')}}
                  ...
                  @endforeach
                  <div>
                    <a href="{{url('/supervisor/addPlanType')}}" class="btn btn-primary add"><i class="fas fa-plus"></i> اضافة نوع</a>
                  </div>
                  </tbody>
                  <tfoot>
                    @if($planTypes->count() > 0)
        ...
        $('.DeleteBtn').click(function(e){
            ...
            .then((willDelete) => {
                if (willDelete) {
                    ...
                    $.ajax({
                        type: "DELETE",
                        url: '/supervisor/deletePlanType/'+id,
                        data: data,
                        // dataType: "data"
