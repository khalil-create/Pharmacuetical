...
{
    use userTrait;
    public function getAllSalesRepresentatives(Request $request)
    {
        if($request->get('id')){
            $this->unreadNotify($request->get('id'));
        }
        $rep = Representative::whereHas('manager')->get();
        return view('managers.marketing.manageSalesRepresentatives', compact('rep',$rep));
    }
    public function addSalesRepresentative()
    {
        ...
        return view('managers.marketing.addSalesRepresentative', compact('mainareas'));              
    }
    public function storeSalesRepresentative(Request $request)
    {
        $request->validate([
        ...
        return redirect('/managerMarketing/manageSalesRepresentatives')->with('status','تم إضافة البيانات بشكل ناجح');
    }
    ...
    public function editSalesRepresentative($id)
    {
        ...
        return view('managers.marketing.editSalesRepresentative',compact('mainareas'))
        ->with('rep',$rep );
    }
    public function updateSalesRepresentative(Request $request,$id)
    {
        $user = User::find($id);
        if(!$user)
        ...
        // $rep->subareas()->syncWithoutDetaching($request->subareasIds);
        return redirect('/managerMarketing/manageSalesRepresentatives')->with('status','تم تعديل البيانات بشكل ناجح');
    }
    public function showSalesRepSubareas($id)
    {
        $rep = Representative::with('user')->find($id);
        $subareas = $rep->subareas;
        return view('managers.marketing.showSalesRepSubareas',compact('subareas'))->with('rep',$rep);
    }
    public function addSalesRepSubareas($id)
    {
        $rep = Representative::with('user')->find($id);
        ...
        return view('managers.marketing.addSalesRepSubareas',compact('subareas'))->with('rep',$rep);
    }
    public function storeSalesRepSubareas(Request $request,$id)
    {
        ...
        return redirect('/managerMarketing/showSalesRepSubareas/'.$id)->with('status','تم اضافة المناطق للمندوب بشكل ناجح');
    }
    ...
    public function showSalesRepDetails($id)
    {
        $rep = Representative::with(['customers','doctors','subareas'])->findOrfail($id);
        if($rep->count() < 1)
            return redirect()->back()->with(['error' => 'هذه البيانات غير موجوده ']);
        return view('Managers.marketing.showSalesRepDetails',compact('rep'));   
    }
}
