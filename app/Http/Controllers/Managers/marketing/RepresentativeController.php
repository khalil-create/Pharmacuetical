...
class RepresentativeController extends Controller
{
    ...
    public function getAllRepresentatives(Request $request)
    {
        ...
        $rep = Representative::with(['user','supervisor'])->whereHas('supervisor')->get();
        return view('Managers.marketing.manageRepresentatives', compact('rep',$rep));
    }
    public function addRepresentative()
    {
        ...
        if($supervisors->count() < 1)
            return redirect()->back()->with(['error' => 'لايمكنك اضافة مندوب علمي قبل مايتم اضافة على الاقل مشرف واحد']);
        
        // $isExistTeamLeader = true;
        // $teamLeader = User::where('user_type','مدير فريق')->get();
        // if($teamLeader->count() < 1)
        //     $isExistTeamLeader = false;
        return view('Managers.marketing.addRepresentative', compact('supervisors'));              
    }
    public function storeRepresentative(Request $request)
    {
        ...
        return redirect('/managerMarketing/manageRepresentatives')->with('status','تم إضافة البيانات بشكل ناجح');
    }
    ...
    public function editRepresentative($id)
    {
        ...
        return view('Managers.marketing.editRepresentative',compact('supervisors'))->with('rep',$rep );
    }
    public function updateRepresentative(Request $request,$id)
    {
        ...
        // }
        // $rep->update();
        return redirect('/managerMarketing/manageRepresentatives')->with('status','تم تعديل البيانات بشكل ناجح');
    }
    public function showSubareas($id)
    {
        $rep = Representative::with('user')->find($id);
        $subareas = $rep->subareas;
        return view('Managers.marketing.showSubareas',compact('subareas'))->with('rep',$rep);
    }
    public function addRepSubareas($id)
    {
        ...
        return view('Managers.marketing.addRepSubareas',compact('subareas'))->with('rep',$rep);
    }
    ...
    //     return view('Managers.marketing.showSubareas',compact('subareas',$subareas))->with('rep',$rep);
    // }
    public function storeRepSubareas(Request $request,$id)
    {
        ...
        return redirect('/managerMarketing/showSubareas/'.$id)->with('stat    us','تم اضافة المناطق للمندوب بشكل ناجح');
    }
    ...
    public function showRepDetails($id)
    {
        ...
        if($rep->count() < 1)
            return redirect()->back()->with(['error' => 'هذه البيانات غير موجوده ']);
        return view('Managers.marketing.showRepDetails',compact('rep'));   
    }
}
