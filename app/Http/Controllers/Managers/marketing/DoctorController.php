...
class DoctorController extends Controller
{
    ...
    public function getAllDoctors(Request $request)
    {
        ...
        $doctors = Doctor::all();
        return view('managers.marketing.manageDoctors',compact('doctors'));
    }
    public function addDoctor()
    {
        $reps = Representative::whereNotNull('supervisor_id')->get();
        if($reps->count() < 1)
            return redirect()->back()->with(['error' => 'لايمكنك اضافة طبيب ولم يتم اضافة على الاقل مندوب واحد']);
        
        $specialists = Specialist::all();
        return view('managers.marketing.addDoctor')->with('reps',$reps)->with('specialists',$specialists);
    }
    public function storeDoctor(Request $request)
    {
        ...
        return redirect('/managerMarketing/manageDoctors')->with('status','تم إضافة البيانات بشكل ناجح');
    }
    ...
    public function editDoctor($id)
    {
        ...
        $reps = Representative::whereNotNull('supervisor_id')->get();
        $specialists = Specialist::all();
        return view('managers.marketing.editDoctor', compact('doctor'))->with('reps',$reps)->with('specialists',$specialists);
    }
    public function updateDoctor(Request $request,$id)
    {
        ...
        $doctor->update();
        $doctor->specialists()->attach($request->specialist_ids);
        return redirect('/managerMarketing/manageDoctors')->with('status','تم تعديل البيانات بشكل ناجح');
    }
    ...
    public function activateDoctor($id)
    {
        ...
        return redirect('/managerMarketing/manageDoctors')->with('status','تم تفعيل الطبيب');
    }
    public function notActivateDoctor($id)
    {
        ...
        return redirect('/managerMarketing/manageDoctors')->with('status','تم إالغاء تفعيل الدكتور');
    }
    public function showDoctorDetails($id)
    {
        ...
        if($doctor->count() < 1)
            return redirect()->back()->with(['error' => 'هذه البيانات غير موجوده ']);
        return view('managers.marketing.showDoctorDetails',compact('doctor'));
    }
}
