...
class ItemController extends Controller
{
    public function getAllItems()
    {
        ...
        return view('managers.marketing.manageItems',compact('items',$items));
    }
    public function addItem()
    {
        $category = Category::all();
        return view('managers.marketing.addItem', compact('category',$category));
    }
    public function storeItem(Request $request)
    {
        ...
        return redirect('/manageItem')->with('status','تم إضافة البيانات بشكل ناجح');
    }
    ...
    public function editItem($id)
    {
        ...
        return view('managers.marketing.editItem', compact('item',$item))->with('categories',$categories);
    }
    public function UpdateItem(Request $request,$id)
    {
        ...
        return redirect('/manageItem')->with('status','تم تعديل البيانات بشكل ناجح');
    }
    public function deleteItem($id)
    {
        ...
        if($item->count() < 1)
            return redirect()->back()->with(['error' => 'هذه البيانات غير موجوده ']);
        $item->delete();
        return redirect('/manageItem')->with('status','تم حذف البيانات بشكل ناجح');
    }
}
