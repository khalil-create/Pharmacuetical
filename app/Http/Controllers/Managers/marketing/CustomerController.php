...
class CustomerController extends Controller
{
    ...
    public function getAllCustomers(Request $request)
    {
        ...
        return view('managers.marketing.manageCustomers',compact('customers'));
    }
    public function addCustomer()
    {
        $SienceReps = Representative::whereNotNull('supervisor_id')->get();
        $SalesReps = Representative::whereNotNull('manager_id')->get();
        if($SienceReps->count() < 1 && $SalesReps->count() < 1)
            return redirect()->back()->with(['error' => 'لايمكنك اضافة عميل ولم يتم اضافة على الاقل مندوب واحد']);
        return view('managers.marketing.addCustomer',compact('SalesReps'))->with('SienceReps',$SienceReps);
    }
    public function storeCustomer(Request $request)
    {
        ...
        $cust->representative()->attach($request->rep_ids);
        return redirect('/managerMarketing/manageCustomers')->with('status','تم إضافة البيانات بشكل ناجح');
    }
    ...
    public function editCustomer($id)
    {
        ...
        if($SienceReps->count() < 1 && $SalesReps->count() < 1)
            return redirect()->back()->with(['error' => 'لايمكنك اضافة عميل ولم يتم اضافة على الاقل مندوب واحد']);
        return view('managers.marketing.editCustomer', compact('SalesReps'))
        ->with('SienceReps',$SienceReps)->with('customer',$customer);
    }
    public function updateCustomer(Request $request,$id)
    {
        ...
        // $customer->representative_id = $request->rep_id;
        $customer->update();
        $customer->representative()->sync($request->rep_ids);
        return redirect('/managerMarketing/manageCustomers')->with('status','تم تعديل البيانات بشكل ناجح');
    }
    public function deleteCustomer($id)
    {
        ...
        // return redirect('/managerMarketing/manageCustomers')->with('status','تم حذف البيانات بشكل ناجح');
    }
    public function activateCustomer($id)
    {
        ...
        ///////////////// Notify user //////////////////////
        $SienceRep = $customer->representative->whereNotNull('supervisor_id')->first();
        $SalesRep = $customer->representative->whereNotNull('manager_id')->first();
        if($SalesRep->count() > 0)
            $this->notifyUser('عملاء','تم تفعيل العميل',$SalesRep->user->id);
        if($SienceRep->count() > 0)
            $this->notifyUser('عملاء','تم تفعيل العميل',$SienceRep->user->id);
        return redirect('/managerMarketing/manageCustomers')->with('status','تم تفعيل العميل');
    }
    public function notActivateCustomer($id)
    {
        ...
        $SalesRep = $customer->representative->whereNotNull('manager_id')->first();
        if($SalesRep->count() > 0)
            $this->notifyUser('عملاء','تم إلغاء تفعيل العميل',$SalesRep->user->id);
        if($SienceRep->count() > 0)
            $this->notifyUser('عملاء','تم إلغاء تفعيل العميل',$SienceRep->user->id);
        return redirect('/managerMarketing/manageCustomers')->with('status','تم إالغاء تفعيل العميل');
    }
    public function showCustomerDetails($id)
    {
        ...
        if($customer->count() < 1)
            return redirect()->back()->with(['error' => 'هذه البيانات غير موجوده ']);
        return view('managers.marketing.showCustomerDetails',compact('customer'));
    }
}
