...
class CategoryController extends Controller
{
    ...
    {
        $cat = Category::whereHas('company')->get();
    ...
    public function storeCategory(Request $request)
    {
        ...
        $company = Company::where('name_company',$request->company_name)->first();
        $mainArea = Category::create([
    ...
    public function UpdateCategory(Request $request,$id)
    {
        ...
        $company = Company::where('name_company',$companyName)->get();
        $compID = 0;
        foreach($company as $s){
            $compID = $s->id;
        }
        $category->name_cat = $request->Input('name_cat');
        $category->company_id = $compID;
        $category->update();
