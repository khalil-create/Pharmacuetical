...
class UsesController extends Controller
{
    ...
    { 
        ...
        $uses = $item->uses;
        if($uses->count() < 1)
            return view('admin.itemUses')->with('item',$item)->with(['error' => 'لم يتم اضافة اي استخدام لهذا الصنف ']);
        return view('admin.itemUses')->with('item',$item);
    }
    public function addUse($id)
    {
        return view('admin.addUse')->with('id',$id);
    }
    public function addUseExist($id)
    {
        $uses = Uses::all();
        return view('admin.addUsesExist')->with('uses',$uses)->with('id',$id);
    }
    ...
    public function editUse($id)
    {
        ...
        if($use->count() < 1)
            return redirect()->back()->with(['error' => 'هذه البيانات غير موجوده ']);
        return view('admin.editUse', compact('use',$use));
    }
    public function updateUse(Request $request,$id)
    {
        ...
        $use->use = $request->use;
        $use->update();
        return redirect('/admin/manageItem/'.$have_category)->with('status','تم تعديل البيانات بشكل ناجح');
    }
